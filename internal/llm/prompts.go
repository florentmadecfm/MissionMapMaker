...
BOUCLES DE REPRISE — cas particulier d'embranchement qui mérite une vérification à part : le texte décrit-il un problème/échec à CORRIGER avant que le processus puisse continuer (ex. "si le paiement échoue", "en cas de stock insuffisant", "si la commande est incomplète, il la complète puis la renvoie") plutôt qu'un simple choix entre deux suites normales qui reconvergent (comme "si le client conteste le montant, le manager valide une remise avant l'encaissement, sinon le serveur encaisse directement" — ici aucune boucle, juste deux chemins qui rejoignent la suite) ? Si le texte implique une action corrective, même en une formulation courte ("on recommence", "on relance", "il la renvoie") : crée une ACTIVITÉ dédiée à cette correction (nouvel élément dans "activities", même si le texte ne lui donne pas un nom formel — choisis un nom d'action clair, ex. "Corriger le paiement", "Compléter la commande"), relie l'activité d'origine à cette nouvelle activité par une interaction dont "condition" porte la raison de l'échec, PUIS ajoute une interaction de RETOUR (sans condition, sauf si le texte précise aussi la condition du retour) depuis cette nouvelle activité vers l'activité à laquelle le processus doit reprendre normalement — typiquement celle-là même qui a échoué. C'est cette interaction de retour qui forme une vraie BOUCLE dans le diagramme (un aller conditionnel suivi d'un retour), au lieu de laisser la branche d'échec en impasse. N'invente jamais cette activité pour un embranchement qui ne décrit aucune correction (un simple choix entre deux suites déjà couvertes par des activités existantes n'a besoin d'aucune boucle).

PREUVES PHYSIQUES — à vérifier pour chaque interaction en parallèle de EMBRANCHEMENTS : le texte mentionne-t-il, ou laisse-t-il clairement deviner, une preuve physique perceptible par le CLIENT à ce moment de l'échange (ex. un reçu papier, un email ou SMS de confirmation, une étiquette, un ticket, une facture) ? Si oui, résume-la en quelques mots dans "physicalEvidence" (pratique du service blueprint). Sinon, laisse "physicalEvidence" vide : ne l'invente jamais pour un échange purement interne entre deux membres de l'équipe (pas de "preuve physique" pour une simple transmission d'information en cuisine, par exemple), seulement quand une trace concrète, perceptible par le client, est réellement en jeu.
