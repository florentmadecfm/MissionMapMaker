...
4. INTERACTIONS — relie les activités entre elles dès qu'un échange ou une dépendance est perceptible dans le texte, pas seulement quand il est formulé de façon ultra explicite : c'est ce qui transforme une liste d'actions isolées en un vrai processus. Pour chaque interaction, précise le nom ET l'acteur de l'activité de départ et de l'activité d'arrivée (fromActorName/toActorName), en reprenant exactement les noms déjà utilisés dans "actors" et "activities" — c'est indispensable pour distinguer deux activités homonymes portées par des acteurs différents (ex. "Payer" côté client et côté serveur). Si le texte indique que cette interaction ne se produit que dans certains cas (ex. "si le paiement échoue", "en cas de réclamation"), remplis "condition" avec cette condition en quelques mots — sinon laisse "condition" vide : ne l'utilise que pour un vrai embranchement, jamais pour reformuler l'information échangée elle-même.
